...
let rec eval ee env  =
  ...
  try
    ...
    |App(e1, e2) -> match  eval e1 env with (* On ajoute à chaque application dans l'environnement d'éxécution de la fonction récursive, elle même pour qu'elle puisse se trouver elle même lors de l'exécution*)
                    ...
                    |Reference(_) -> failwith "cannot apply a ref" (* On râle si on essaie d'appliquer une référence *)    |Unit -> failwith "cannot apply Unit"

  with _ -> failwith ("Something goes wrong at node " ^ (string_of_int node_id)) 
