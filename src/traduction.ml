...
let rec trad_expr ee =
  ...
  try match e with
     (* | Const k -> Fun(s, Cart(Const k, s))
      ...
      | Let((x, e1), e2) -> let e1 = trad_expr e1 and e2 = trad_expr e2 in Fun(s, Let(Cart(x,s1), App(e1, s), App(e2, s1)))
      | LetRec((x, e1), e2) -> let e1 = trad_expr e1 and e2 = trad_expr e2 in Fun(s, LetRec(Cart(x,s1), App(e1, s), App(e2, s1)))
      | App(e1,e2) -> let e1 = trad_expr e1 and e2 = trad_expr e2 in Fun(s, Let(Cart(f1s1), App(e1, s), Let(Cart(v2,s2), App(e2, s1), App(App(f1, v2), s2))))
(*Là il y a un truc pénible car on n'a pas les booléens en fouine. Il va falloir modifier ce truc pour généraliser à tous les opérateurs*)                                                 
  ...
  (*aspects impératifs*)
      ...
      | Ref(e) ->Fun(s, Let(Cart(v,s1), App(e, s), Let(Cart(l,s2), App(App(alloc, v), s1), Cart(ls2))))
      | x -> Fun(s, Cart(x,s))
