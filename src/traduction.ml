...
let rec trad_expr ee =
  ...
  try match e with
      | Add(e1,e2) -> let  s0 = news () in let s1 = news() in let s2 = news() in  let v1 = newv () in let v2 = newv () in
let te1 =  trad_expr e1 in let te2 = trad_expr e2 in mkLetPair(v1,s1, mkApp(te1, s0), mkLetPair(v2,s2, mkApp(te2, s1), mkPair(Add(v1,v2),s2)))
